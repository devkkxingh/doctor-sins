...
const vertexShader = `
  ...
  void main() {
    ...
    // Calc Noise for Portal Ring
    float angle = atan(targetPos.y, targetPos.x);
    float dist = length(targetPos.xy);
    
    float noiseVal = snoise(vec2(angle * 4.0, uTime * 2.0 + 1.5 * 5.0));
    float distortion = 1.0 + (noiseVal * 0.05); 
    
    vec3 noisyTarget = targetPos;
    noisyTarget.x *= distortion;
    noisyTarget.y *= distortion;

    // Expand from Center (0,0) to Ring Radius
    vec3 pPos = noisyTarget * uGather; 

    // Mix Sequence
    vec3 currentPos = mix(startPos, hPos, uHandMix);
    
    // OVERRIDE: If gathering, move from center
    // We want them to look like they are emitted from center
    if (uGather > 0.01) {
        // As uGather goes 0->1, we transition from Universe to Portal
        // But we want the visual "source" to be center
        // So we mix from Center(0) to pPos?
        // No, we want them to *be* pPos, which is already scaling from 0.
        // The issue is mix(startPos, ..., uGather).
        // At low uGather, it dominates by startPos.
        // We need to switch to pPos faster.
        
        float portalInfluence = smoothstep(0.0, 0.2, uGather); // Rapid transition
        currentPos = mix(currentPos, pPos, portalInfluence);
    } 
    
    // Wobble (Additional jitter to make it sparkle)
...
export function Starfield() {
    ...
    useFrame((_, delta) => {
        ...
        if (rawIsGesturing && !isFist) {
            ...
            // HIGH DECAY for "instantly go away"
            // If Fist, decay even faster (Instant shatter)
            const decay = isFist ? 10.0 : 4.0;
            s.stabilityScore -= dt * decay;
        }
